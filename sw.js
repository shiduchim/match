const CACHE='peermatch-v21';
const SHELL=['./','./index.html','./manifest.webmanifest','./icon.svg','./whatsapp-enhance.js','./peermatch-v11.js','./ui-v18.js','./peermatch-v19.js','./audio-v21.js'];
...
async function withEnhancer(response){
  ...
  let html=text;
  if(!html.includes('whatsapp-enhance.js')) html=html.replace('</body>','<script src="./whatsapp-enhance.js?v=21"></script></body>');
  if(!html.includes('peermatch-v11.js')) html=html.replace('</body>','<script src="./peermatch-v11.js?v=21"></script></body>');
  if(!html.includes('ui-v18.js')) html=html.replace('</body>','<script src="./ui-v18.js?v=21"></script></body>');
  if(!html.includes('peermatch-v19.js')) html=html.replace('</body>','<script src="./peermatch-v19.js?v=21"></script></body>');
  if(!html.includes('audio-v21.js')) html=html.replace('</body>','<script src="./audio-v21.js?v=21"></script></body>');
  ...
  if(e.request.method==='POST'&&u.pathname.endsWith('/share-target')){
    e.respondWith((async()=>{
      try{
        ...
        return Response.redirect('./?pmv=21&shared=1',303);
      }catch(err){
