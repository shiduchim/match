const CACHE='peermatch-v19';
const SHELL=['./','./index.html','./manifest.webmanifest','./icon.svg','./whatsapp-enhance.js','./peermatch-v11.js','./ui-v18.js','./peermatch-v19.js'];
...
async function withEnhancer(response){
  ...
  let html=text;
  if(!html.includes('whatsapp-enhance.js')) html=html.replace('</body>','<script src="./whatsapp-enhance.js?v=19"></script></body>');
  if(!html.includes('peermatch-v11.js')) html=html.replace('</body>','<script src="./peermatch-v11.js?v=19"></script></body>');
  if(!html.includes('ui-v18.js')) html=html.replace('</body>','<script src="./ui-v18.js?v=19"></script></body>');
  if(!html.includes('peermatch-v19.js')) html=html.replace('</body>','<script src="./peermatch-v19.js?v=19"></script></body>');
  ...
  if(e.request.method==='POST'&&u.pathname.endsWith('/share-target')){
    e.respondWith((async()=>{
      try{
        ...
        return Response.redirect('./?pmv=19&shared=1',303);
      }catch(err){
