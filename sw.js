...
self.addEventListener('activate',e=>{
  e.waitUntil((async()=>{
    ...
    await Promise.all(keys.filter(k=>k.startsWith('peermatch-v')&&k!==CACHE).map(k=>caches.delete(k)));
...
async function saveShare(req){
  ...
  await new Promise((resolve,reject)=>{
    ...
    tx.onerror=()=>reject(tx.error||new Error('Share import failed'));
    tx.onabort=()=>reject(tx.error||new Error('Share import failed'));
  });
}
