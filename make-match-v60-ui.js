/* PeerMatch v127: clearer Make Match flow + PDF-first profile sharing.
   - Uses authoritative ID-based selection from peermatch-v11.js.
   - Android text-only WhatsApp opens the installed app directly.
   - If either selected profile has an attached PDF, WhatsApp/Email shares the actual
     PDF and omits that profile's OCR/autofilled body from the outgoing message.
   - Removing the PDF automatically returns that profile to normal text sharing. */
(function(){
  ...
  style.textContent=`
    ...
    .pmV60SendTo{margin-top:9px!important}.pmV60SendTo select{margin-top:5px}
    ...
    .pmChatDetail .event{background:#f7fafc!important;border-color:#d9e4ea!important}.pmChatDetail .event+.event{margin-top:8px!important}
  ...
  function visibleFor(k){
    ...
    return (data[k]||[]).filter(x=>{
      const hay=k==='shadchanim'
        ...
        :`${x.name||''} ${x.age||''} ${x.text||''} ${x.lookingFor||''} ${x.lookingForMaxAge||''} ${senderName(x)} ${senderPhone(x)} ${(x.activities||[]).map(a=>a.text||a.action||'').join(' ')}`;
      return hay.toLowerCase().includes(q);
    });
  }

  function checkedRecords(k){
    if(typeof window.pmGetSelected==='function'){try{return window.pmGetSelected(k)||[];}catch(e){}}
    const listId=k==='shadchanim'?'shadchanList':k+'List',list=document.getElementById(listId);if(!list)return[];
    const arr=visibleFor(k),boxes=[...list.querySelectorAll('.pmListCheck')],out=[];
    boxes.forEach((box,i)=>{if(box.checked&&arr[i])out.push(arr[i]);});return out;
  }
  ...
  function attachmentType(x){
    ...
    return t.includes('pdf')||n.endsWith('.pdf')?'application/pdf':t;
  }
  function hasPdf(x){return x?.profileAttachment instanceof Blob&&attachmentType(x)==='application/pdf';}
  function pdfFile(x,label){
    if(!hasPdf(x))return null;const b=x.profileAttachment;
    let name=String(x.profileAttachmentName||'').trim()||safePart(cleanName(x,label||'profile'))+'.pdf';if(!/\.pdf$/i.test(name))name+='.pdf';
    try{return b instanceof File&&/\.pdf$/i.test(b.name||'')?b:new File([b],name,{type:'application/pdf',lastModified:Date.now()});}catch(e){return null;}
  }
  function pdfFiles(guy,girl){return [pdfFile(guy,'Guy'),pdfFile(girl,'Girl')].filter(Boolean);}
  ...
  function profileBlock(x,label){
    ...
    if(hasPdf(x))lines.push('Profile attached as PDF.');
    else{const text=String(x?.text||'').trim();if(text)lines.push(text);}
  ...
  function canShareFiles(files){if(!files.length||typeof navigator.share!=='function')return false;if(!navigator.canShare)return true;try{return navigator.canShare({files});}catch(e){return false;}}
  function downloadFile(file){const u=URL.createObjectURL(file),a=document.createElement('a');a.href=u;a.download=file.name||'profile.pdf';a.style.display='none';document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),60000);}
  ...
  function whatsappTarget(phone,message){
    ...
    return /Android/i.test(navigator.userAgent||'')?'whatsapp://send?phone='+encodeURIComponent(d)+'&text='+encodeURIComponent(message||''):'https://wa.me/'+d+'?text='+encodeURIComponent(message||'');
  }
  ...
  async function addHistory(guy,girl,shadchan,recipient,channel,message,contactOnly){
    ...
    try{await save();return true;}catch(e){console.warn('PeerMatch v127 history save',e);return false;}
  }
  ...
  async function shareWithPhotos(channel,guy,girl,shadchan,recipient,message){
    ...
    if(!canShareFiles(files))return directSend(channel,guy,girl,shadchan,recipient,message);
    const title='Shidduch suggestion: '+cleanName(guy,'Guy')+' & '+cleanName(girl,'Girl');
    try{await navigator.share({title,text:message,files});}catch(e){if(e?.name==='AbortError')return;console.warn('PeerMatch v127 photo share',e);return directSend(channel,guy,girl,shadchan,recipient,message);}
  ...
  async function shareWithPdfs(channel,guy,girl,shadchan,recipient,message,includePhotos){
    const pdfs=pdfFiles(guy,girl);if(!pdfs.length)return includePhotos?shareWithPhotos(channel,guy,girl,shadchan,recipient,message):directSend(channel,guy,girl,shadchan,recipient,message);
    if(channel!=='whatsapp'&&channel!=='email')return alert('Attached PDF profiles can be sent from Make Match with WhatsApp or Email.');

    const photos=includePhotos?photoFiles(guy,girl):[],all=[...pdfs,...photos];
    let files=canShareFiles(all)?all:(canShareFiles(pdfs)?pdfs:[]);
    if(!files.length){
      pdfs.forEach(downloadFile);
      alert('This browser could not attach the profile PDF automatically. The PDF was saved to your device; attach it manually after the message opens.');
      return directSend(channel,guy,girl,shadchan,recipient,message);
    }

    const title='Shidduch suggestion: '+cleanName(guy,'Guy')+' & '+cleanName(girl,'Girl');
    try{await navigator.share({title,text:message,files});}
    catch(e){if(e?.name==='AbortError')return;console.warn('PeerMatch v127 PDF Make Match share',e);pdfs.forEach(downloadFile);alert('The PDF was saved to your device. Attach it manually after the message opens.');return directSend(channel,guy,girl,shadchan,recipient,message);}
    if(!await addHistory(guy,girl,shadchan,recipient,channel,message,false))alert('The match was shared, but PeerMatch could not save it in History.');
    try{render();}catch(e){}close();
  }
  ...
  function openMakeMatchV60(){
    ...
    const photos=photoFiles(guy,girl),hasAnyPdf=hasPdf(guy)||hasPdf(girl);
    const mediaLabel=photos.length?'<label class="pmV60Photos"><input id="pmV60IncludePhotos" type="checkbox"> <span>'+(hasAnyPdf?'Include profile photos too':'Include profile photos with WhatsApp / Email')+'</span></label>':'';
    const pdfHint=hasAnyPdf?' Profiles with an attached PDF are sent as the PDF; their OCR/autofilled text is not included.':'';
    open(`<h2>Make match</h2><div class="pmV60Form"><div class="pmV60Chosen"><div class="pmV60ChosenTitle">Suggested match</div><div class="pmV60Pair"><div class="pmV60Person"><span>Guy</span><strong>${esc(cleanName(guy,'Guy profile'))}</strong></div><div class="pmV60Person"><span>Girl</span><strong>${esc(cleanName(girl,'Girl profile'))}</strong></div></div><div class="pmV60Shad"><span>Shadchan</span><strong>${shadchan?esc(cleanName(shadchan,'Shadchan')):'Not selected'}</strong></div></div><label class="pmV60SendTo">Send to<select id="pmV60Recipient">${contacts.map((r,i)=>`<option value="${i}">${esc(recipientDisplay(r)+(r.phone?' • '+r.phone:''))}</option>`).join('')}</select></label><label>Message<textarea id="pmMatchMessage"></textarea></label>${mediaLabel}<div class="pmV60Actions"><button id="pmV60Contact" type="button">Contact</button><button id="pmMatchSMS" type="button">SMS</button><button id="pmMatchWA" type="button">WhatsApp</button><button id="pmMatchEmail" type="button">Email</button></div><div class="pmV60Hint">Contact calls the person selected above. SMS, WhatsApp and Email send the message to that same selection.${esc(pdfHint)}</div><button id="pmV60Cancel" class="secondary full" type="button">Cancel</button></div>`);
    ...
    const send=channel=>{
      const text=String(message.value||'').trim();if(!text)return alert('Enter a message.');
      const include=!!document.getElementById('pmV60IncludePhotos')?.checked;
      if(hasAnyPdf){if(channel==='sms')return alert('An attached PDF cannot be sent automatically by SMS. Use WhatsApp or Email, or remove the PDF to use the regular text profile.');return shareWithPdfs(channel,guy,girl,shadchan,current(),text,include);}
      if(include&&(channel==='whatsapp'||channel==='email'))return shareWithPhotos(channel,guy,girl,shadchan,current(),text);
      return directSend(channel,guy,girl,shadchan,current(),text);
    };
