/* PeerMatch v60: clearer Make Match flow + stronger History separation.
   ...
   - A selected Shadchan is first/default.
   - Renames Call to Contact and follows the current Send to selection.
   - Removes the separate Share photo button; optional photos remain available inline.
   - Default message starts with the suggested pair, uses clearer profile breaks,
     says "Shadchan <name>" in the greeting, and has no trailing stock sentence.
   - Saves Make Match activity to both Guy and Girl history records.
   - Gives History a distinct background/header treatment inside detail screens.
*/
(function(){
  ...
  style.textContent=`
    ...
    @media(max-width:390px){
      .pmV60Pair{gap:5px}
      .pmV60Person{padding:7px}
      .pmV60Actions{gap:4px}
      .pmV60Actions button{font-size:9.2px!important;padding:9px 1px!important}
    }
  `;
  ...
  function contactOptions(guy,girl,shadchan){
    const out=[];
    if(shadchan){
      out.push({kind:'shadchan',label:'Shadchan',name:cleanName(shadchan,'Shadchan'),greeting:'Shadchan '+cleanName(shadchan,'Shadchan'),phone:String(shadchan.phone||'').trim(),email:String(shadchan.email||'').trim(),side:'Shadchan'});
    }
    const add=(side,x)=>{
      const phone=senderPhone(x);if(!phone)return;
      const name=senderName(x);
      out.push({kind:'contact',label:side+' contact person',name:name||side+' contact person',greeting:name||side+' contact person',phone,email:'',side});
    };
    add('Girl',girl);
    add('Guy',guy);
    return out;
  ...
  function recipientDisplay(r){
    if(!r)return'contact person';
    return r.name&&r.name!==r.label?r.label+' — '+r.name:r.label;
  }
  ...
  function profileBlock(x,label){
    ...
    if(sn||sp)lines.push('Contact person: '+(sn||'Contact person')+(sp?' • '+sp:''));
    return lines.join('\n');
  }
  ...
  function defaultMessage(guy,girl,recipient){
    ...
    return [
      ...
      '',
      '--------------------',
      profileBlock(guy,'Guy'),
      '--------------------',
      profileBlock(girl,'Girl'),
      '--------------------'
    ].join('\n');
  }
  ...
  function photoFiles(guy,girl){
    const files=[];
    const add=(side,x)=>{
      const photo=fullPhoto(x);if(!(photo instanceof Blob))return;
      const f=toPhotoFile(photo,side+'_'+safePart(cleanName(x,side))+'_photo'+extFor(photo.type));if(f)files.push(f);
    };
    add('Guy',guy);add('Girl',girl);return files;
  }

  function waNumber(phone){
    const raw=String(phone||'').trim();let d=raw.replace(/\D/g,'');if(!d)return'';
    if(raw.startsWith('+'))return d;if(d.startsWith('00'))d=d.slice(2);if(d.startsWith('0'))d='972'+d.slice(1);return d;
  }
  ...
  async function addHistory(guy,girl,shadchan,recipient,channel,message,contactOnly){
    const g=canonical('guys',guy.id),l=canonical('girls',girl.id),s=shadchan?canonical('shadchanim',shadchan.id):null;
    if(!g||!l)return false;
    const matchId=Date.now(),ts=stamp();
    const gName=cleanName(g,'Guy profile'),lName=cleanName(l,'Girl profile'),who=recipientDisplay(recipient);
    ...
    try{await save();return true;}catch(e){console.warn('PeerMatch v60 history save',e);return false;}
  }
  ...
  async function directSend(channel,guy,girl,shadchan,recipient,message){
    ...
    let target='';
    if(channel==='whatsapp'){
      const d=waNumber(recipient.phone);if(!d)return alert('The selected contact needs a phone number for WhatsApp.');
      target='https://wa.me/'+d+'?text='+encodeURIComponent(message);
    }else if(channel==='sms'){
      if(!recipient.phone)return alert('The selected contact needs a phone number for SMS.');
      target='sms:'+recipient.phone+'?body='+encodeURIComponent(message);
      ...
      target='mailto:'+encodeURIComponent(recipient.email)+'?subject='+encodeURIComponent(subject)+'&body='+encodeURIComponent(message);
    }
  ...
  async function shareWithPhotos(channel,guy,girl,shadchan,recipient,message){
    ...
    if(!files.length)return directSend(channel,guy,girl,shadchan,recipient,message);
    ...
    catch(e){if(e?.name==='AbortError')return;console.warn('PeerMatch v60 photo share',e);return directSend(channel,guy,girl,shadchan,recipient,message);}
  ...
  function openMakeMatchV60(){
    ...
    open(`<h2>Make match</h2><div class="pmV60Form">
      <div class="pmV60Chosen">
        <div class="pmV60ChosenTitle">Suggested match</div>
        <div class="pmV60Pair">
          <div class="pmV60Person"><span>Guy</span><strong>${esc(cleanName(guy,'Guy profile'))}</strong></div>
          <div class="pmV60Person"><span>Girl</span><strong>${esc(cleanName(girl,'Girl profile'))}</strong></div>
        </div>
        <div class="pmV60Shad"><span>Shadchan</span><strong>${shadchan?esc(cleanName(shadchan,'Shadchan')):'Not selected'}</strong></div>
      </div>
      <label class="pmV60SendTo">Send to<select id="pmV60Recipient">${contacts.map((r,i)=>`<option value="${i}">${esc(recipientDisplay(r)+(r.phone?' • '+r.phone:''))}</option>`).join('')}</select></label>
      <label>Message<textarea id="pmMatchMessage"></textarea></label>
      ${photos.length?'<label class="pmV60Photos"><input id="pmV60IncludePhotos" type="checkbox"> <span>Include profile photos with WhatsApp / Email</span></label>':''}
      <div class="pmV60Actions">
        <button id="pmV60Contact" type="button">Contact</button>
        <button id="pmMatchSMS" type="button">SMS</button>
        <button id="pmMatchWA" type="button">WhatsApp</button>
        <button id="pmMatchEmail" type="button">Email</button>
      </div>
      <div class="pmV60Hint">Contact calls the person selected above. SMS, WhatsApp and Email send the message to that same selection.</div>
      <button id="pmV60Cancel" class="secondary full" type="button">Cancel</button>
    </div>`);
    ...
    const rebuild=()=>{message.value=defaultMessage(guy,girl,current());};
    ...
    document.getElementById('pmV60Cancel').onclick=close;
  }

  document.addEventListener('click',e=>{
    const btn=e.target?.closest?.('.pmSendMatchTop');
    if(!btn)return;
