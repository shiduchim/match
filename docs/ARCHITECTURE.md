...
- **Images** render directly via an object URL in `<img>`, inside a full-screen in-app overlay (`#pmAttachmentViewer`) with a Share/Save button — unchanged behavior.
- **PDFs (and anything non-image)**, as of v112, are handed to the OS instead of rendered in-app: `shareOrDownloadAttachment(x, blob, type)` builds a real `File` from the Blob with its original name/MIME type, tries `navigator.share({files:[file]})` (letting Android's native share sheet offer any installed PDF-capable app), and falls back to a named `<a download>` click (Chrome's download manager writes it to Android's Downloads folder) if sharing is unavailable. An in-app PDF.js renderer was tried at v111 and still failed on the user's device, so it was removed rather than patched further — do not reintroduce in-app PDF rendering without a specific reason to revisit this decision (see `docs/DECISIONS.md`).

Never `window.open(blobUrl)`, never a new tab, never a Blob URL navigation for opening an attachment — that produced a broken `blob://localhost/...` page on Android and is why both the v111 and v112 changes happened. The saved attachment itself is never touched by any of this — a failed share/download attempt only shows an `alert()`, it never deletes data.
