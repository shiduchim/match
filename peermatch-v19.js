...
(function(){
  ...
  /* The proven phone-normalization + wa.me URL behavior, verified working on the installed
     Android PWA via the ordinary Shadchan-detail WhatsApp button below (compose(x,'WhatsApp')).
     Any other file that needs to open WhatsApp to a specific number with prefilled text must
     call this instead of re-deriving its own wa.me URL/normalization. Returns '' if the phone
     doesn't normalize to anything usable. */
  window.pmWhatsAppUrl=function(phone,text){
