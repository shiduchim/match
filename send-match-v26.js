/* PeerMatch v26: Send Match uses the existing list checkboxes. Shadchan is optional. */
(function(){
  document.documentElement.dataset.peerMatchVersion='26';
  ...
  function senderRecipients(guy,girl){
    ...
    const add=(side,x)=>{
      ...
      out.push({key:side,label:(senderName(x)||side+' sender'),phone:p,side});
    };
    add('Guy',guy);add('Girl',girl);
    // Avoid duplicate phone choices if the same person sent both profiles.
  ...
  function addHistory(guy,girl,shadchan,recipient,channel,message){
    ...
    const meta={matchId,guyId:guy.id,girlId:girl.id,shadchanId:shadchan?.id||null,channel,message,recipient:recipientLabel,recipientPhone:recipient?.phone||shadchan?.phone||''};
  ...
  async function handoff(channel,guy,girl,shadchan,recipient,message){
    ...
    try{await save();}catch(e){console.warn('PeerMatch v26 match history save',e);return alert('PeerMatch could not save the match history. Nothing was opened.');}
    try{render();}catch(e){}
    close();
  ...
  function openSendMatch(){
    ...
    const chosenLabel=shadchan?cleanName(shadchan,'Shadchan'):(recipients[0]?.label||'');
    ...
    open(`<h2>Send match</h2><div class="pmMatchForm">
      ...
      <div class="pmMatchHint">The Guy and Girl came from your existing checkbox selections. A Shadchan is optional. PeerMatch saves the exact message in both profile histories, and also in the Shadchan history when one is selected.</div>
    ...
    const rebuild=()=>{
      const label=shadchan?cleanName(shadchan,'Shadchan'):(currentRecipient()?.label||chosenLabel);
  ...
  function installHeader(){
    ...
    // Remove controls from older versions, including Backup.
