/* PeerMatch v126: email backup as a share-safe text attachment.
   Android Chrome can reject ZIP files in Web Share even though image attachments work.
   PeerMatch now wraps the exact restore ZIP as base64 inside a text/plain file, which
   Gmail accepts as a real attachment. Restore Backup also accepts that emailed .txt
   wrapper and transparently converts it back to the original ZIP before restore. */
(function(){
  document.documentElement.dataset.peerMatchVersion='126';
  ...
  function bytesToB64(bytes){let out='';const size=0x8000;for(let i=0;i<bytes.length;i+=size){const part=bytes.subarray(i,Math.min(i+size,bytes.length));let s='';for(let j=0;j<part.length;j++)s+=String.fromCharCode(part[j]);out+=btoa(s);}return out;}
  ...
  async function prepare(email,state){
    ...
    catch(e){state.file=null;email.textContent='Email backup';if(progress)progress.textContent='';console.warn('PeerMatch v126 email backup preparation',e);}
  ...
  function bindEmail(){
    const email=document.getElementById('pmV124EmailBackup');if(!email||email.dataset.pmV126Email==='1')return;
    const state={file:null,preparing:false,sharing:false};states.set(email,state);email.dataset.pmV126Email='1';email.textContent='Email backup';
    email.onclick=async e=>{
      ...
      catch(err){if(err?.name!=='AbortError'){console.warn('PeerMatch v126 email backup share',err);alert('PeerMatch could not attach the email backup. Try again.');}}
  ...
  function bindRestore(){
    const input=document.getElementById('pmRestoreFile');if(!input||input.dataset.pmV126Restore==='1'||typeof input.onchange!=='function')return;
    input.dataset.pmV126Restore='1';input.accept='.zip,.txt,application/zip,text/plain';const original=input.onchange;
    input.onchange=async e=>{
      const f=input.files?.[0];if(!f)return;
      if(!/\.txt$/i.test(f.name||''))return original.call(input,e);
      try{const text=await f.text();if(!text.startsWith(HEADER))return original.call(input,e);const b64=text.slice(HEADER.length).replace(/\s+/g,''),bytes=b64ToBytes(b64),zipName=(f.name||'PeerMatch_Backup.txt').replace(/\.txt$/i,'.zip'),zip=new File([bytes],zipName,{type:'application/zip'}),dt=new DataTransfer();dt.items.add(zip);input.files=dt.files;return original.call(input,e);}catch(err){console.warn('PeerMatch v126 emailed backup restore',err);alert('PeerMatch could not read this emailed backup file.');}
    };
  }
